...
pub fn encode(allocator: Allocator, input: []const u8) ![]const u8 {
    ...
    var buf_arr: std.Io.Writer.Allocating = try .initCapacity(allocator, capacity);
    defer buf_arr.deinit();
    var w_arr = buf_arr.writer;

    var str = input;
    while (std.mem.indexOfAny(u8, str, encode_chars)) |index| {
        w_arr.writeAll(str[0..index]) catch unreachable;
        const value_decoded = try std.unicode.utf8Decode(&[_]u8{str[index]});
        w_arr.print("&#{d};", .{value_decoded}) catch unreachable;

        const start_next = index + 1;
        str = str[start_next..];
    }
    w_arr.writeAll(str) catch unreachable;
    return w_arr.buffered();
}
