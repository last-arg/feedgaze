...
// TODO?: Request is freed by caller?
pub fn makeRequest2(arena: *ArenaAllocator, url: []const u8, headers: zfetch.Headers) !*zfetch.Request {
    var req = try zfetch.Request.init(arena.allocator(), url, null);
    try req.do(.GET, headers, null);
    return req;
}

pub fn resolveRequest2(arena: *ArenaAllocator, url: []const u8, headers_slice: []zfetch.Header) !*zfetch.Request {
    try zfetch.init();
    ...
    var current_url = url;
    var req = try makeRequest2(arena, url, headers);
    var redirect_count: u16 = 0;
    while (redirect_count < max_redirects) : (redirect_count += 1) {
        switch (req.status.code) {
            301, 307, 308 => {
                ...
                req.deinit();
                req = try makeRequest2(arena, current_url, headers);
            },
            else => break,
        }
    }
...
test "resolveRequest2()" {
    ...
    // url redirects
    const req = try resolveRequest2(&arena, "http://localhost:8080/rss2", &headers);
